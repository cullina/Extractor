...
allEquivalent max x = map (\n -> rotateBy max n x) [0..max-1]

minHammingDist max x y = minimum $ map (hammingDist x) (allEquivalent max y)


maxDist max length x y = minHammingDist max x y == length - 1


allReducedVectors max length = map (0:) $ allVectors max (length - 1)
